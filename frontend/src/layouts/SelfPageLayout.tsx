...
const SelfPageLayout = ({ className, title, data, type }: SelfPageLayoutProps) => {
    const [itemsPerPage, setItemsPerPage] = useState<number>(5)
    const [isLoading, setIsLoading] = useState<boolean>(false)
    const [currentData, setCurrentData] = useState<typeof data>(data.slice(0, itemsPerPage));
    ...
    return (
        <main className={className + "  padding-y padding-x"}>
            <h1 className="text-2xl capitalize mb-8 font-bold text-center">{title}</h1>

            <div>
                <div className="flex flex-wrap gap-5">
                    {

                        currentData.map((datum) => (type === 'course' ? <CourseCard
                            date={datum.date}
                            id={datum.id}
                ...
                <div className="flex justify-center mt-10">
                    <Button disabled={isLoading} className="rounded-full" onClick={handleLazyLoad}>
