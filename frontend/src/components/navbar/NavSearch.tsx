...
const fetchSuggestions = async (query: string): Promise<string[]> => {
  ...
  return query ? allSuggestions.filter((suggestion) => suggestion.toLowerCase().includes(query.toLowerCase())) : []
}
...
const NavSearch = ({
  ...
  const renderSearchButton = () => (
    <Button
      variant="ghost"
      size="icon"
      className={cn("rounded-full hover:bg-secondary", searchFloatButtonClassName)}
          ...
          suggestions?.map((suggestion, index) => (
            <div key={index} className="flex items-center hover:bg-muted/50 px-3 py-2">
              ...
              >
                ...
                <span className="truncate">{shortenSuggestion(suggestion)}</span>
              </Button>
            </div>
          ))
        )}
  ...
  const renderSearchHistory = () => (
    <div className="w-full absolute bg-background border rounded-b-lg shadow-md z-50">
      <ScrollArea className="max-h-[60vh]">
        <div className="py-2">
          <div className="px-4 py-1 text-sm font-medium text-muted-foreground">Recent searches</div>
          ...
          {historyLoading ? (
            ...
            searchHistory?.map((item) => (
              <div key={item.id} className="flex items-center hover:bg-muted/50 px-3 py-2">
                <Button
                  variant="ghost"
                ...
                >
                  ...
                  <span className="truncate">{shortenSuggestion(item.query)}</span>
                </Button>
                <Button
                  variant="ghost"
                  size="icon"
          ...
          render={({ field }) => (
            <div className="relative" ref={searchContainerRef}>
              <FormItem>
                <div
                  className={cn(
                    "flex items-center rounded-full border bg-background overflow-hidden h-10",
                    { "p-2": isFullscreen },
                    className,
                  )}
                >
                  ...
                  {!isFullscreen && <Search className="h-4 w-4 ml-3 text-muted-foreground" />}
  ...
  if (floating === "always") {
    return <div className={cn("relative", containerClassName)}>{renderSearchSheet()}</div>
  }
  ...
  return <div className={cn("w-full", containerClassName)}>{renderSearchForm(false, false)}</div>
}
