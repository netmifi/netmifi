...
// Importing queryState constants from the specified file.

const User = require('../models/User');
// Importing the User model from the specified file.

const jwt = require('jsonwebtoken');
// Importing the jsonwebtoken library to handle JWT operations.

const { cookieOptions } = require('../constants/cookieOptions');
// Importing cookie options constants from the specified file.

const verifyJwt = (req, res, next) => {
// Defining the verifyJwt middleware function which takes req, res, and next as arguments.

    const cookies = req.cookies;
    // Extracting cookies from the request object.

    const token = cookies?.jwt;
    // Extracting the JWT token from the cookies, if it exists.
